...
class sq_density():
    ...
    def __init__(self, conn_obj, filename = PATH + 'dens_per_hour.csv',):
    ...
    def update_actual_ids(self, conn_obj):
        ...
        q = 'select showplace_id from e_and_s_schema.showplace_tbl where release_version = 1'
        
        _ = conn_obj.open_conn()
        self.ids_list = pd.read_sql_query(q, conn_obj.conn)['showplace_id'].values.tolist()
        _ = conn_obj.close_conn()
  
    def update_head_pop(self, const_obj, part = 0.4):
        # part - доля самых популярных
        # СЕЙЧАС ЗАГЛУШКА - вручную сформированный список, один для всех
        ...
        self.head_pop = const_obj.pop_list
        
    def update_tail_pop(self, const_obj, part = 0.4):
        ...
        self.tail_pop = [x for x in self.tail_pop if x not in const_obj.pop_list]
...
class CONST_VAL():
    ...
    def __init__(self):
        self.lunch_time = 2
        # self.pop_list = [10034, 10234, 10165, 10026, 10030, 10050, 10088, 10045, 10066] # tripadvisor
        self.pop_list = [10027, 10237, 10235, 10026, 10234, 4, 10033, 10210, 10238] # 10083 old release
...
#             print('start_vis_hour',start_vis_hour)
            ...
            while start_cur_hour< 17 and len(adj_mat.columns)>0 and order<4:
#                 print(order)
                if order!=1:
#                     print(adj_mat)
...
dens_obj.update_head_pop(const_obj)
dens_obj.update_tail_pop(const_obj)
